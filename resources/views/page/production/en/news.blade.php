@extends('en-welcome')
@section('title','Perfect Companion Indonesia - News & Events')
@section('content')
							...
							<div class="row sm:fw-w">
								<div class="col col-3 sm:col-12 sm:mb-6">
									<div class="ff-2 p lh-6 tc-dark-contrast mr-20 sm:mr-0">
										<span class="pos-r">What's the story today? <br />
											<span class="pos-a -r-4 t-0 h-100% d-flex ai-end"><i class="fas fa-caret-right"></span></i>Let's find out.
										</span>
									</div>
								</div>
								<div class="col col-9 sm:col-12">
									<div class="h4 ff-1-bd tc-dark-contrast -mt-4 sm:h4 sm:max-w-100% sm:ff-1">
										<span><span class="ff-1">News & Events</span> <br class="sm:d-none" />Perfect Companion Indonesia</span>
									</div>
								</div>
							</div>
						</div>
					...
					<div class="d-flex">
						<div class="tab-header mb-10">
							...
							<div class="tab-header-item active" id="all">
								<span class="p-lg">All</span>
							</div>
							<?php
							foreach ($category as $key => $value) {
							?>
								<div class="tab-header-item ml-3" id=<?= $value->id ?>>
									<span class="p-lg txtf-c"><?= $value->name ?></span>
								</div>
							<?php } ?>
										...
										<div class="col col-3 mb-10 sm:col-12">
											<div class="px-2 pb-4">
												<div class="news-slider-item make_transition cur-p" id="news-detail" data-transition-to="{{ url('en/news-detail/'.$value->id) }}">
													...
													<p class="p-xl lc-3 ff-1 tc-dark px-2"><?= $value->title ?></p>
												</div>
											</div>
										</div>
									<?php } ?>
