@extends('en-welcome')
@section('title','Perfect Companion Indonesia - All Products')
@section('content')
...
<div class="content-wrapper sub-page all-page" id="product-all-page">
	<div class="content">
		...
		<div class="fullwidth-master section-1 section-header sm:h-100vh sm:d-flex sm:ai-end">
			<div class="header-slider-item lazy pos-r max-w-100% mx-10 mt-40 sm:mx-0" style="background-image: url('<?php echo url('/assets/images/banner/product-all-header.jpg') ?>">
							...
							<div class="row sm:fw-w">
								...
								<div class="col col-9 sm:col-12">
									<div class="h1 ff-1-bd tc-dark-contrast max-w-80% -mt-4 sm:h2 sm:max-w-100%">
										<span>The Right Choice<br> For Your Lovely Pet.</span>
									</div>
								</div>
							</div>
						</div>
			...
			<div class="inner">
				<div class="t-n-s mb-10 sm:mb-4">
					<div class="d-flex ai-center op-80% mb-2">
						...
						<div>
							<p class="p-sm ff-2-bd ml-4 tc-primary-500">This is All</p>
						</div>
					</div>
					<div class="h3 ff-1-bd tc-primary-500 txtf-c sm:h4">Our Products.</div>
				</div>
				<div class="row sm:fw-w">
							...
							<div class="pr-10">
									...
									<div class="filter-option-wrapper -ml-4">
										<?php foreach ($value['options'] as $key => $item) { ?>
											<div class="filter-option-item ml-3 mb-3 filter-<?= $value['title'] ?>" id="<?= $item->id ?>">
												<span class="p-md txtf-c"><?= $value['title'] == "Brand" ? $item->name : $item->type ?></span>
											</div>
										<?php } ?>
									</div>
								<?php }
								...
								<div class="d-flex fd-col mt-6 apply-filter-wrapper">
									...
									<button class="btn bg-primary-500 tc-link-contrast h-12 px-10 mt-8 bd-n bd-rs-8 d-flex ai-center jc-center hidden btn-filter" id="submit">
										<span class="text ff-1-bd">Apply Filter</span>
									</button>
						...
						<div class="row fw-w -ml-2 -mr-2 sm:fw-w" id="result-product">
							...
							?>
								<div class="col col-3 mb-10 sm:col-6">
									<div class="px-2">
										<div class="product-item <?= $value->type ?> make_transition cur-p" id="product-detail" data-transition-to="{{ url('en/product-detail/'.$value->id_product) }}">
											...
											<div class="product-text-wrapper px-2">
												<div class="d-flex ai-end jc-between mb-3 sm:mb-0">
													<div class="p-xl ff-1-bd sm:p-lg lc-1"><?= $value->product ?></div>
												</div>
												...
												<?php foreach ($value->variants as $k => $var) { ?>
													{{-- <span class=" variant-item d-flex ai-center jc-center px-2 h-6 bd-1 bd-solid bd-c-primary-300 bd-rs-3 mb-2 max-w-75% mr-2" data-key="<?= $k ?>">
																<p class="p-xs ff-2 lc-1 tc-primary-500"><?= $var->name ?></p>
															</span> --}}
												<?php
												}
												?>
