...
<div class="d-sm-flex align-items-center justify-content-between mb-4">
    <h1 class="h3 mb-0 text-gray-800">Varian</h1>
</div>

<!-- DataTales Example -->
<div class="card shadow mb-4">
    <div class="card-header py-3">
        <h6 class="m-0 font-weight-bold text-primary">Form - Varian </h6>
    </div>
    <div class="card-body row">
        <div class="col-xl-8 col-lg-7">
            <form method="POST" class="user" accept-charset="utf-8" enctype="multipart/form-data" action="<?= ($action == 'create') ? url('save-variant/'.$id_product) : url('update-variant/'.$data['id'].'/'.$id_product) ?>">
                @if ($action == 'edit')
                @method('patch')
                @endif
                @csrf
                ...
                <div class="form-group">
                    ...
                    <div>
                        <div class="img-wp">
                            <img name="{{ $value['field'] }}" id="{{ $value['field'] }}" class="@error($value['field']) is-invalid @enderror " src="<?= url('assets/lampiran/'.$value['field'])?>/{{ $data[$value['field']] }}">
                        </div>
                    </div>
                ...
                <a href="<?php echo url('/list-variant/'.$id_product) ?>" class="btn btn-secondary">Kembali</a>
            </form>
        </div>
    </div>
</div>
